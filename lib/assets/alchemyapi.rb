...
class AlchemyAPI
    ...
    # use ENV variables instead of api_key.txt to use config vars and deploy to heroku
    ...
    #
		# 	@apiKey = key
		# rescue => err
		# 	#The file doesn't exist, so show the message and result the file.
		# 	puts 'API Key not found! Please copy/paste your API key into the file: api_key.txt'
		# 	puts 'If you do not have an API Key from AlchemyAPI please register for one at: http://www.alchemyapi.com/api/register.html'
		#
		# 	#result a blank file to hold the key
...
# Writes the API key to api_key.txt file. It will result the file if it doesn't exist.
